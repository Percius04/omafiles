...
Beta 3 shipped the three features an audit identified as most likely to make a Hyprland power user switch: **content search**, **custom actions**, and **path autocomplete**. What's next, roughly:

- **Toward v1.0 (stable):** full session restore (scroll + active panel + history across restarts, today just paths), persistent "open with" per file type, empty-trash-from-UI + trash size, and a fuzzy jump (recents + bookmarks + history with ranking).
- **v1.1:** an operation queue (pause/resume/retry), a network-credentials prompt, and optional search filters (`type:`/`size:`/`date:`).

Deliberately **out of scope** (they'd erode the Omarchy/DHH minimalism): configurable view modes, an icon-size/settings panel, and anything that turns Omafiles into "Nautilus again."
